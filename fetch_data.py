...
def fetch_financial_data(ticker, period="2d", interval="1h"):
    try:
        ...
        if hist_data.empty or len(hist_data) < 5:  # Need at least 5 hours for momentum
            print(f"Insufficient data for {ticker}: {len(hist_data)} hours")
            return None, None, None
        current_price = hist_data["Close"].iloc[-1]
        # 5-hour momentum
        initial_price = hist_data["Close"].iloc[-5]  # 5th hour back
        ...
        volatility = returns.std() * np.sqrt(252 * 24) * 100 if len(returns) > 1 else None
...
def calculate_composite_score(results):
    try:
        ...
        inputs = {
            ...
            "VIX_Volatility": (results.get("VIX_Volatility"), 5, 30, 0.05),
            "GVZ_Level": (results.get("GVZ_Current"), 10, 40, 0.15),
            "GVZ_Momentum": (results.get("GVZ_Momentum"), -5, 5, 0.10),
            "GVZ_Volatility": (results.get("GVZ_Volatility"), 5, 30, 0.05),
            "DXY_Level": (results.get("DXY_Current"), 80, 120, 0.10),
            "DXY_Momentum": (results.get("DXY_Momentum"), -5, 5, 0.05),
            "DXY_Volatility": (results.get("DXY_Volatility"), 5, 30, 0.05),
            "GOLD_Level": (results.get("GOLD_Current"), 2000, 4000, 0.10),
            "GOLD_Momentum": (results.get("GOLD_Momentum"), -2, 2, 0.15),
            "GOLD_Volatility": (results.get("GOLD_Volatility"), 5, 20, 0.10),
        }
